...
A soft hyphen at a paragraph or hard-break start is consumed, but the hard break
after it still ends a line: `a`, LF, soft hyphen, LF, `b` in pre-wrap paints an
empty second line in Chrome, Safari and Firefox, as do two soft hyphens there and
two such chunks in a row. Pretext used to drop a chunk that a line start consumed
whole, its hard break included; the line start now takes a hard break that ends a
chunk holding nothing else as an empty line. At the end of the text, with no hard
break after it, Chrome and Safari still give a chunk of soft hyphens a line and
Firefox doesn't (ENGINE_FOLLOWUPS.md).
