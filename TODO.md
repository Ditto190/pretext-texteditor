...
- In Safari, `prepare()` measures each distinct word before a space again together with that space. Measuring once per final grapheme would be cheaper, but only where the final glyph doesn't depend on context: not for joining scripts or fonts with contextual alternates, such as Shantell Sans.
