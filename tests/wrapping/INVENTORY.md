...
- **Content-language observations:** `maintained/content-language` runs 28 shapes on `en`, `ja`, `ko`, `zh` and `zh-Hant` pages, plus five language-independent controls on `en`, in `full` only: 145 rows per browser. They cover small kana and `ー` after `？`, `！`, ideographs and kana; curly double and single quotes around Latin and CJK text; `〜` and `゠`; and normal-mode newlines between wide characters or next to East Asian punctuation, with Hangul, emoji, `₩` and word-joiner controls. Each width is a measured prefix plus 0.1px that ends the first line at the tested boundary. Preparation receives no language, so the rows are research scope and require nothing. The documented normalization still collapses those newlines to spaces; only the raw native rectangles show Firefox's removal.
...
Retained imports preserve public text, settings and thresholds, rather than porting every archived harness's language and font-loading protocol. The bounded-round and policy recipe harnesses used an absent document language. Nine older imported cohorts used `<html lang="en">`: under `policy-correctness/`, these are `discretionary/chrome-tab-threshold-v1`, `discretionary/chrome-shy-threshold-v2`, `measurement/tab-shy-v2`, `measurement/emoji-context-matrix-firefox-v1`, `chromium-script-spacing/item-graphemes-v1`, `chromium-script-spacing/fresh-context-v1`, and `tabs/chrome-v4`, `tabs/safari-v4`, `tabs/firefox-v4`. Their archived `run.ts` files establish that context; `fixtures/provenance.json` identifies the imported reports. All these inputs remain runnable under the shared observer. The shared fixture document uses `en`, so those nine cohorts run with their archived document language; their font-loading protocol still differs. With Chinese as the macOS preferred language, absent and `en` document language differ in Chrome curly-quote breaks and Firefox fallback widths, so current results do not certify a complete replay of the absent-language protocols. An empty element `lang` explicitly marks the language unknown, so unlike an absent element `lang` it does not inherit the fixture document's `en`. Chrome resolves it to its app language, as it does for a document without a language, and Firefox's fallback glyphs follow the machine language too. Empty-language reset inputs therefore follow the machine in Chrome and Firefox; they remain inputs rather than being rewritten to `en`.
...
| 8 — content language, locale and dictionary behavior | Imported language/locale and Myanmar inputs, explicit language recipes, content-language page observations, maintained corpora. Empty content language remains a real reset input. |
