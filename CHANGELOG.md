...
- Lines no longer break before closing punctuation such as `，`, `。`, `！` or `）` after Latin letters or digits, as in `xxxx，b`. A time followed by a full-width comma, such as `00:00:00，`, now stays whole instead of breaking after a `:`.
