...
- A zero-width joiner at the start of a paragraph, after a zero-width space, or after a tab or line break in `pre-wrap` now stays with the character after it (#233).
...
- In Safari, a next-line character (U+0085) now stays on the same line as the text before it, and lines can still break after it. `letterSpacing` no longer adds space after U+0085, except next to text that Safari shapes as complex text, such as Arabic, Devanagari or a combining mark.
- In Safari, a tab in `white-space: pre-wrap` now moves to the following tab stop when less than half a space would remain before the next one, as Safari does.
