...
- In Chrome, when the hyphen of a chosen soft hyphen does not fit, the line now ends at an earlier space, zero-width space or soft hyphen that leaves room for it, as Chrome does, instead of overflowing. With `letterSpacing`, Chrome's visible hyphen no longer gets its own letter spacing (#PR).
