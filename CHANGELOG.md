...
- In `white-space: pre-wrap`, a line holding only soft hyphens before a newline, at the start of the text or after another newline, now counts as a line, as browsers draw it, instead of disappearing. In Safari, so does a line holding only soft hyphens or spaces between two line or paragraph separators (U+2028, U+2029).
