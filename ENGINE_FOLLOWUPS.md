...
- Take a content language for line breaking (approved). Installed observations (`maintained/content-language`, RESEARCH.md) show Safari's small-kana, `ー` and quote rules following `ja` and `ko` pages, Chrome's quote, `〜` and `゠` rules following `zh` pages, and Firefox's newline removal next to East Asian punctuation following `ja` and `zh` pages. Build it on the generated line-break class table, keep `setLocale()` segmenter-only, add no expensive browser work to `prepare()` or `layout()`, and rerun the family in each installed browser before each layer. Decide whether the language comes only from `<html lang>` or also from a `prepare()` option.
