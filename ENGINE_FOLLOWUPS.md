...
- On every page, Chrome breaks after a closing curly quote before CJK text (`他说“你好”` / `然后走了`), while Pretext's closing-quote carry keeps the CJK attached. No browser treats curly single quotes around Latin text as brackets, and Firefox doesn't treat double quotes as brackets, but Pretext does.
...
- Gate the Blink profile's CJ resolution to ID and the Gecko profile's to NS in installed Chrome and Firefox. A Bun count over `generateCases` moves units in 256 ordinary `maintained/accuracy` rows per engine (`Japanese short`: `ー` in Chrome, small kana in Firefox), in 25 Chrome and 20 Firefox `maintained/content-language/cj` rows, and in 61 Firefox `maintained/corpus` rows of `ja-kumo-no-ito`, which loses 64 units.
- After a digit or Latin letter, as in `約3ヶ月` or `日本abcァア`, the profiles that resolve CJ to NS (Safari on other pages, Firefox and unrecognized engines) still let small kana and `ー` start a line, because only CJK text takes a following piece that starts with CJ. Observe the `cj/digit-*` and `cj/latin-*` rows of `maintained/content-language` in installed Safari on `en` and `ja` pages and in Firefox; if they keep them, attach that piece after any text, not only CJK text.
