...
- Follow the page language in the remaining line-break rules (approved). Preparation reads `<html lang>` once and resolves it to `ja`, `ko`, `zh` or root, with no `prepare()` option; only Safari's small-kana and `ー` rule uses it so far. Rerun the family and the installed Safari gate before making those rows required: Japanese text on `en` fixture pages now keeps small kana with the character before them, which changes Safari's units in 256 ordinary `maintained/accuracy` rows, while the Japanese corpora, prepared on `ja` pages, keep theirs. Installed observations (`maintained/content-language`, RESEARCH.md) show Safari's quote rules following `ja` pages, Chrome's quote, `〜` and `゠` rules following `zh` pages, and Firefox's newline removal next to East Asian punctuation following `ja` and `zh` pages. Build them on the generated line-break class table, keep `setLocale()` segmenter-only, add no expensive browser work to `prepare()` or `layout()`, and rerun the family in each installed browser before each layer.
- On every page, Chrome breaks after a closing curly quote before CJK text (`他说“你好”` / `然后走了`), while Pretext's closing-quote carry keeps the CJK attached. No browser treats curly single quotes around Latin text as brackets, and Firefox doesn't treat double quotes as brackets, but Pretext does. Pretext's Firefox profile lets small kana start a line, which Firefox never allows.
...
- Chrome lets `ー` (CJ) start a line on every page, but the Blink profile still keeps it from starting one except after `！` or `？`. After installed runs, switch the Blink and Gecko profiles to `conditionalJapaneseStarterModel: 'resolved'`, which also keeps Firefox's small kana from starting a line, and drop the field. For Gecko that changes 64 `ja-kumo-no-ito` units; that corpus has no `ー`.
- On every page, the Safari profile still lets small kana and `ー` start a line after a digit or Latin letter, as in `約3ヶ月` or `日本abcァア`, because only CJK text takes a following piece that starts with CJ. Observe the `cj/digit-*` and `cj/latin-*` rows of `maintained/content-language` in installed Safari on `en` and `ja` pages; if Safari keeps them, let the resolved model attach that piece after any text, not only CJK text.
