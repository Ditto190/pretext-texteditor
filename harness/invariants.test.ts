...
const PLANTS: ReadonlyArray<readonly [string, Profile, string, string, ReadonlyArray<readonly [RegExp, string]>, string]> = [
  ...
  ['a last line ending at segment Infinity would have layoutNextLine build its text without end, and a JSON copy of that end start the paragraph again', 'unknown', 'infinite-end', 'layout.ts',
  ...
  ['a rich fragment that ends its item at segment Infinity would have materializeRichInlineLineRange build its text without end', 'unknown', 'rich-infinite-end', 'rich-inline.ts',
